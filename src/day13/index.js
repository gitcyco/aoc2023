...
function findMirror(arr) {
  for (let y = 1; y < arr.length; y++) {
    let found = true;
    let boundary = Math.min(arr.length - y, y);
...
function findSmudgedMirror(arr) {
  for (let y = 1; y < arr.length; y++) {
    let smudges = 0;
    let boundary = Math.min(arr.length - y, y);
    for (let i = 0; i < boundary; i++) {
      smudges += lev(arr[y + i], arr[y - i - 1]);
      if (smudges > 1) {
        break;
      }
    }
